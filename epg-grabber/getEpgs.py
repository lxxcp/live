...
def process_channels(epg_data, mapping, tvg_ids, processed_channels, root_today, root_four_days): 
...
def process_programs(epg_data, mapping, tvg_ids, today_start, four_days_end, root_today, root_four_days): 
    valid_ids = set(tvg_ids.keys())  
    program_count = 0 
...
def filter_and_build_epg(urls, mapping, tvg_ids): 
    ...
    for url in urls: 
        ...
        # 处理频道 
        channel_count = process_channels(epg_data, mapping, tvg_ids, processed_channels, root_today, root_four_days) 
        logging.info(f" 从 {url} 处理 {channel_count} 个频道") 
 
        # 处理节目 
        program_count = process_programs(epg_data, mapping, tvg_ids, today_start, four_days_end, root_today, root_four_days) 
        logging.info(f" 从 {url} 